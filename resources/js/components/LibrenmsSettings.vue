...
<template>
  <tabs @tab-selected="tabChanged" :selected="this.tab">
    ...
    <tab
      name="global"
      :selected="'global' === tab"
      :text="$t('settings.groups.global')"
    >
      <ul class="settings-list">
        <li
          v-for="setting in settings"
          :key="setting.name"
          v-show="settingShown(setting.name)"
        >
          <strong>{{ setting.name }}</strong>
          <code>{{ setting.type === 'password' ? '******' : setting.value }}</code>
        </li>
      </ul>
    </tab>

    <tab
      v-for="(sections, group) in groups"
      :key="group"
      :name="group"
      :selected="group === tab"
      :text="$t('settings.groups.' + group)"
    >
      <accordion @expanded="sectionExpanded" @collapsed="sectionCollapsed">
        <accordion-item
          v-for="(items, item) in groups[group]"
          :key="item"
          :name="item"
          :text="$t('settings.sections.' + group + '.' + item + '.name')"
          :active="item === section"
        >
          <template
            v-if="$te('settings.sections.' + group + '.' + item + '.description')"
          >
...
export default {
  ...
  methods: {
    ...
    settingShown(setting_name) {
      ...
      if (setting.when.hasOwnProperty("and")) {
        return setting.when.and.reduce(
          (result, logic) => this.checkLogic(logic) && result,
          true
        );
      } else if (setting.when.hasOwnProperty("or")) {
        return setting.when.or.reduce(
          (result, logic) => this.checkLogic(logic) || result,
          false
        );
      }
    ...
    translatedCompare(prefix, a, b, suffix) {
      return this.$t(prefix + a + suffix).localeCompare(
        this.$t(prefix + b + suffix)
      );
    },
    checkLogic(logic) {
  ...
  mounted() {
    ...
    axios.get(route("settings.list")).then((response) => {
  ...
  computed: {
    groups() {
      if (_.isEmpty(this.settings)) {
        let sorted_tabs = {};
        this.tabs
          .sort((a, b) => this.translatedCompare("settings.groups.", a, b))
      ...
      Object.keys(groups)
        ...
        .forEach((group_key) => {
          sorted[group_key] = {};
          Object.keys(groups[group_key])
            .sort((a, b) =>
              this.translatedCompare(
                "settings.sections." + group_key + ".",
                a,
                b,
                ".name"
              )
            ...
            .forEach((section_key) => {
              sorted[group_key][section_key] = _.sortBy(
                groups[group_key][section_key],
                "order"
              ).map((a) => a.name);
            });
        });
